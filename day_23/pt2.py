...
def compute_pt1(str_input, moves=100):
    ...
    for ii in range(moves):
        ...
        #str_input = list(set(str_input).difference(popped_elems))
...
inp = list(map(int, ex_input))
input_ = inp + list(range(int(max(inp)), 1000000 + max(inp) - len(inp), 1))

str_input = compute_pt1(input_, 100)
idx_1 = str_input.index(1)
print(str_input[idx_1: idx_1+3])
#breakpoint()
